...
<div class="container">
                            ...
                            echo '<tr  style="'.$bgcolor.'">
									<td style="text-align:center;">
										<div class="btn-group">
											...
											<button title="'.$titleestado.' Categoria" class="btn btn-'.$colestado.'" onclick="desactivar('.$value->{'id'}.')" > <span class="fas fa-'.$estado.'"></span></button>
											<button title="Editar Categoria" class="btn btn-warning" onclick="edita('.$value->{'id'}.')" > <span class="fas fa-pen"></span></button>
	...
	<script type="text/javascript">
		function Agregar(){
			$("#Modal_titulo_addCategoria").html("Agregar Categoria");
			$('#Modal_addCategoria').modal();
            document.getElementById("Agregar").reset();
		}
	</script>
	<div class="modal fade" id="Modal_addCategoria" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel2" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="Modal_titulo_addCategoria">Agregar Categoria</h5>
          <button class="close" type="button" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
          </button>
        </div>
        <div class="modal-body" id="Modal_body_addCategoria">
        	<form id="Agregar">
        		<input type="hidden" name="request" value="CrearCategoria">
  ...
  <script type="text/javascript">
  	...
  	function edita(id){
			...
			$("#Modal_titulo_addCategoria").html("Modificar Categoria");
    ...
    function desactivar(id){
        swal({
            title: "Estas segur@?",
            text: "Estas apunto de desactivar la Categoria!",
            icon: "warning",
            buttons: [
                'No, Cancelar!',
                'Si, estoy seguro!'
            ...
            if (isConfirm) {
                $("#Modal_titulo_addCategoria").html("Desactivar Categoria");
                $.post('Backend/Controller.php', {
                    request: 'Listar',
                    tabla: 'getCategoria',
                    id: id
   ...
   $.ajax({
      ...
      success: function(respuesta){
          ...
          if (respuesta.status=='Error') {
              swal({
                  title: "Error",
                  text: "No se guardo el Categoria, intentelo nuevamente.",
                  icon: "error"
              })
      }else{
      		$('#Modal_addCategoria').modal('hide');
          swal({
              title: "Éxito",
              text: "Se guardo el Categoria.",
              icon: "success",
          }).then((seguro) => {
              cargar('AdminCategoria','adm/Categoria.php','Controller')
          });
      }
